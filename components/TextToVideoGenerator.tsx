...
const TextToSpeechGenerator = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Save the audio and caption files in the correct location
      const audioResponse = await fetch(audioUrl);
      const audioBlob = await audioResponse.blob();
      const audioFile = new File([audioBlob], "speech.wav", { type: "audio/wav" });

      const timingResponse = await fetch(timingUrl);
      const timingBlob = await timingResponse.blob();
      const timingFile = new File([timingBlob], "cap_tim.json", { type: "application/json" });

      // Trigger the video creation process
      const formData = new FormData();
      formData.append("audio", audioFile);
      formData.append("captions", timingFile);
